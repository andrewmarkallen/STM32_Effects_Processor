...
{ 
	...
	for(i=0; i<BLOCK_SIZE; i++)
	{
		...
		if((int32_t)*rw_index < 0) *rw_index += n_order + 1;
		if(*rw_index > n_order) *rw_index -= n_order + 1;
		x_n[*rw_index] = x_n[*rw_index + n_order + 1] = comp->in[0][i];
		float d_n = comp->in[1][i];

		// Apply FIR filter
		if(apply || *update)
		{
			arm_dot_prod_f32(w_n, x_n + *rw_index, n_order, &sum);
		...
		if(*update)
		{
			arm_dot_prod_f32(a_n, x_n + *rw_index + 1, n_order, &e);
			e = x_n[*rw_index] - e;
			...
			c = k_ext_n[n_order];
			delta = e * k_ext_n[0] - c * x_n[*rw_index + n_order];
